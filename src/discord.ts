...
export const EM_LOG_BATCH_SIZE = appConfig.compactOutput ? 5 : 10;
export const EM_LOG_DELAY = 2000; // 2 seconds, 30 batches per minute (webhook ratelimit)

const workingBatches: string[][] = [];

export const handleEMLogLine = (data: string) => {
  ...
  if (!currBatch || currBatch.length >= EM_LOG_BATCH_SIZE) {
    workingBatches.push([]);
